...
class VNEEnv(gym.Env):
    ...
    def __init__(self, substrate_network_instance, vnr_generator_func, 
        ...
        self._last_processed_vnr_for_wrapper = None 
        self.current_vnr_ordered_vnns_to_place = deque() # VNNs to be placed, in specific order
        self.current_vnn_to_embed = None 
        self.vn_to_sn_mapping_current_vnr = {} 
        self.temp_sn_node_reservations = {} 
        self.temp_sn_link_reservations = [] 

        self.max_vnr_nodes_obs = max_vnr_nodes_obs 
        self.max_sn_nodes_obs = max_sn_nodes_obs   
        ...
        self.substrate_nodes_list = [] 
        self.node_id_to_idx = {}       

        self.current_step_in_vnr_embedding = 0
        self.max_steps_per_vnr_episode = (self.max_vnr_nodes_obs * 2) + 5 # Max VNNs + Max Links (approx) + buffer

        self.total_successful_vnrs = 0
        self.total_failed_vnrs = 0
    ...
    def _calculate_node_rank(self, sn_node_id):
        ...
        total_adj_bw = 0
        for neighbor in sorted(list(self.sn.network.neighbors(sn_node_id))): 
            if self.sn.network.has_edge(sn_node_id, neighbor):
                 total_adj_bw += self.sn.network.edges[sn_node_id, neighbor].get('bandwidth', 0)
        rank = (self.alpha_rank * available_cpu) + (self.beta_rank * total_adj_bw)
        return rank
    ...
    def _get_observation(self):
        ...
        obs_parts = []
        vnn = self.current_vnn_to_embed
        norm_vnn_cpu = np.clip(vnn['cpu'] / 50.0, 0, 1) 
        obs_parts.extend([ norm_vnn_cpu,
            1.0 if 'ground' in vnn['candidate_domains'] else 0.0,
            1.0 if 'air' in vnn['candidate_domains'] else 0.0,
            1.0 if 'leo' in vnn['candidate_domains'] else 0.0,
        ])
        ...
        for sn_id_in_list in self.substrate_nodes_list:
            ...
            if sn_id_in_list not in self.sn.network: 
                obs_parts.extend([0.0] * 7); sn_node_features_count += 1; continue
            sn_data = self.sn.network.nodes[sn_id_in_list]
            rank = self._calculate_node_rank(sn_id_in_list) 
            is_used_by_this_vnr = 1.0 if sn_id_in_list in self.vn_to_sn_mapping_current_vnr.values() else 0.0
            norm_cpu = np.clip(sn_data.get('cpu',0) / 100.0, 0, 1) 
            adj_bw = self.sn.get_total_adjacent_bandwidth(sn_id_in_list) 
            norm_adj_bw = np.clip(adj_bw / 500.0, 0, 1) 
            norm_rank = np.clip(rank / 300.0, 0, 1) 
            obs_parts.extend([ norm_cpu, norm_adj_bw, norm_rank, 
                1.0 if sn_data.get('type') == 'ground' else 0.0,
                1.0 if sn_data.get('type') == 'air' else 0.0,
                1.0 if sn_data.get('type') == 'leo' else 0.0,
                is_used_by_this_vnr
            ]); sn_node_features_count += 1
        while sn_node_features_count < self.max_sn_nodes_obs:
            obs_parts.extend([0.0] * 7); sn_node_features_count += 1
        final_obs = np.array(obs_parts, dtype=np.float32)
        if final_obs.shape[0] != self.observation_space.shape[0]:
            padded_obs = np.zeros(self.observation_space.shape, dtype=np.float32)
            len_to_copy = min(final_obs.shape[0], self.observation_space.shape[0])
            padded_obs[:len_to_copy] = final_obs[:len_to_copy]
            return np.clip(padded_obs, 0.0, 1.0) 
    ...
    def reset(self, seed=None, options=None):
        ...
        # Prepare VNNs for placement in their generated path order
        self.current_vnr_ordered_vnns_to_place.clear() # Use the renamed deque
        if self.current_vnr and 'path_ordered_vnn_ids' in self.current_vnr and 'nodes' in self.current_vnr:
            ...
            for vnn_id_in_path in self.current_vnr['path_ordered_vnn_ids']:
                if vnn_id_in_path in vnn_objects_map:
                    self.current_vnr_ordered_vnns_to_place.append(vnn_objects_map[vnn_id_in_path])
                # else:
                    # print(f"Warning: VNN ID '{vnn_id_in_path}' from path_ordered_vnn_ids not found in VNR nodes list.")
        
        self.vn_to_sn_mapping_current_vnr = {} 
        self.temp_sn_node_reservations = {} 
        self.temp_sn_link_reservations = [] 

        if self.current_vnr_ordered_vnns_to_place:
            self.current_vnn_to_embed = self.current_vnr_ordered_vnns_to_place.popleft()
        else: 
            self.current_vnn_to_embed = None 
            if self.current_vnr and self.current_vnr.get('nodes'): # If VNR has nodes but path order failed
                # This might happen if path_ordered_vnn_ids is empty for a multi-node VNR
                print(f"Warning: VNR {self.current_vnr.get('id')} has nodes but could not form processing queue from path_ordered_vnn_ids.")
    ...
    def step(self, action):
        ...
        if self.current_vnn_to_embed:
            ...
            if valid_placement:
                ...
                reward += 0.1; info['vnn_placement_status'] = f"VNN {vnn['id']} tentatively_placed_on {chosen_sn_id}"
            else: 
                reward -= 0.1; info['vnn_placement_status'] = f"VNN {vnn['id']} failed_placement_on {chosen_sn_id if chosen_sn_id else 'invalid_idx'}: {placement_error}"
            
            if self.current_vnr_ordered_vnns_to_place: # Use the renamed deque
                self.current_vnn_to_embed = self.current_vnr_ordered_vnns_to_place.popleft()
            else: 
                self.current_vnn_to_embed = None 
        
        if self.current_vnn_to_embed is None: 
            # Ensure current_vnr and its nodes list exist before checking length
            ...
            if len(self.vn_to_sn_mapping_current_vnr) == expected_nodes_count and expected_nodes_count > 0 :
                ...
                if links_successfully_embedded:
                    self._finalize_vnr_embedding(successful=True)
                    reward = self.base_success_reward 
                    info['vnr_status'] = "successfully_embedded"
                else:
                    self._finalize_vnr_embedding(successful=False, reason=f"link_embedding_failed: {reason}")
                    reward = -1.0; info['vnr_status'] = f"failed_link_embedding ({reason})"
            elif expected_nodes_count == 0 and not self.vn_to_sn_mapping_current_vnr : # VNR had no nodes initially
                self._finalize_vnr_embedding(successful=True) # No nodes/links to embed, so "success"
                reward = 0.1 # Small reward for processing an empty (but valid) VNR
                info['vnr_status'] = "successfully_processed_empty_vnr"
            else: # Not all VNNs could be mapped, or VNR was invalid
                self._finalize_vnr_embedding(successful=False, reason="incomplete_vnn_mapping")
                reward = -1.0; info['vnr_status'] = "failed_incomplete_vnn_mapping"
            terminated = True 
        ...
        if self.current_step_in_vnr_embedding >= self.max_steps_per_vnr_episode and not terminated:
            ...
            if info.get('vnr_status') is None : 
                 self._finalize_vnr_embedding(successful=False, reason="max_steps_reached")
                 reward = -1.0; info['vnr_status'] = "failed_max_steps_reached"
    ...
    def _embed_links_for_current_vnr(self):
        if not self.current_vnr: return False, "no_current_vnr_for_links" # Should not happen
        vnr_links_original = self.current_vnr.get('links', [])
        if not vnr_links_original: return True, "no_links_to_embed"
        vnr_links_sorted = sorted(vnr_links_original, key=lambda l: (l.get('from',''), l.get('to',''))) # Added .get for safety
        current_vnr_link_allocations_temp = []
        g_temp_view = self.sn.network.copy()
        for vnl in vnr_links_sorted: 
            vnn_from_id = vnl['from']; vnn_to_id = vnl['to']; bw_req = vnl['bandwidth']
            sn_from_node = self.vn_to_sn_mapping_current_vnr.get(vnn_from_id)
            sn_to_node = self.vn_to_sn_mapping_current_vnr.get(vnn_to_id)
            if not sn_from_node or not sn_to_node: return False, f"vnn_not_mapped_for_link_{vnl.get('id','N/A')}"
            if sn_from_node == sn_to_node: 
                current_vnr_link_allocations_temp.append({'vnl_id': vnl.get('id', f"{vnn_from_id}-{vnn_to_id}"),'path': [sn_from_node],'bw_reserved': 0})
                continue
            ...
            if path:
                ...
                for i in range(len(path) - 1):
                    u, v = path[i], path[i+1]
                    if g_temp_view.has_edge(u,v): g_temp_view.edges[u,v]['bandwidth'] -= bw_req
    ...
    def _bfs_find_path(self, graph_view, sn_source_id, sn_target_id, bw_required):
        ...
        if bw_required == 0 and sn_source_id != sn_target_id :
             if graph_view.has_edge(sn_source_id, sn_target_id): return [sn_source_id, sn_target_id]
        ...
        while q:
            ...
            for neighbor in sorted_neighbors: 
                if neighbor not in visited:
                    if graph_view.has_edge(curr, neighbor): 
                        edge_data = graph_view.edges[curr, neighbor]
                        if edge_data.get('bandwidth', 0) >= bw_required:
                            visited.add(neighbor); new_path = list(path); new_path.append(neighbor)
                            q.append((neighbor, new_path))
        return None

    def _finalize_vnr_embedding(self, successful, reason=""):
        vnr_id_to_finalize = "N/A_finalize"
        if self.current_vnr and 'id' in self.current_vnr: # Ensure current_vnr and its id exist
            vnr_id_to_finalize = self.current_vnr['id']
        ...
        self.temp_sn_link_reservations = []
        self.current_vnn_to_embed = None
        if hasattr(self, 'current_vnr_ordered_vnns_to_place'): 
            self.current_vnr_ordered_vnns_to_place.clear()
        # self.current_vnr is set to None (or new VNR) by reset()
    ...
    def calculate_revenue_for_vnr(self, vnr_details, successfully_embedded):
        ...
        revenue = sum(node.get('cpu', 0) for node in vnr_details.get('nodes', [])) + \
                  sum(link.get('bandwidth', 0) for link in vnr_details.get('links', []))
    ...
    def calculate_cost_for_vnr(self, vnr_details, successfully_embedded, vnr_id_if_successful=None):
        if not successfully_embedded or not vnr_details: return 0.0
        cost = 0.0
        for node in vnr_details.get('nodes', []): cost += node.get('cpu', 0)
        if vnr_id_if_successful and vnr_id_if_successful in self.sn.embedding_mapping:
            embedded_links_info = self.sn.embedding_mapping[vnr_id_if_successful].get('links', [])
            for link_alloc in embedded_links_info:
            ...
            for link_details in vnr_details.get('links', []): cost += link_details.get('bandwidth', 0) 
